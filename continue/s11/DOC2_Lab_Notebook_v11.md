...
**v11 · Experiments, code, and architecture decisions**
...
myLLM/
  data/                   ← raw and processed datasets
    ...
    input.txt             ← Tiny Shakespeare dataset (downloaded Session 12)
  ...
  model/                  ← model architecture files
    ...
    train.py              ← training loop (created Session 12, debugged Session 13)
    generate.py           ← text generation from checkpoint (created Session 13)
  runs/                   ← training checkpoints
    best_model.pt         ← saved checkpoint from best validation loss
...
| Vocabulary size               | 65                       | Confirmed from Tiny Shakespeare dataset — matches placeholder |
...
| Property               | Details                                                              |
| ---------------------- | -------------------------------------------------------------------- |
| Dataset name / source  | Tiny Shakespeare — all of Shakespeare's works concatenated           |
| Raw size               | ~1MB                                                                 |
| After cleaning         | 1,115,394 characters                                                 |
...
| Dataset: Tiny Shakespeare             | Session 12      | Standard reference dataset for this architecture size. ~1MB, 65 unique characters, ~1.1M tokens. vocab_size = 65 matches the placeholder already in transformer.py — no code changes needed.                                                                                                  |
...
_End of Document 2 — Lab Notebook — v11_
