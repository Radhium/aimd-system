Phase 2 begins — PyTorch Tensors
...
That second superpower has a name: autograd. We will get to it after you feel comfortable with the tensors themselves.
...
print(a.device)
Before you run it — tell me: what do you predict a.shape, b.shape, and a.device will print?a.shape = (3,)b.shape = (2,2)this I can't predict. it says cpubut wait why cpu? should it not be in gpu then ?11:29 amYour shape predictions are exactly right. And your question about the device is exactly the right question to ask — that instinct is good.
...
Have a think, then run it and tell me what happened.I though a_gpu would be in gpucpuuser@Shree-Ji-PC MINGW64 /c/projects/myLLM
...
cpu
cuda:0
cpubut then again why cpu for the 1st one?11:32 amYour predictions were both correct. And I can see exactly what confused you — let me clear it up.

Why does the first one still say CPU?
Look at the order of your print statements in the code:
pythonprint(a.device) # you printed this BEFORE moving it — says cpu
a_gpu = a.to("cuda")
...
Does that fully resolve the confusion, or is any part of it still fuzzy? Once you are solid on this, we move to the most important part — autograd.I want to know why it says "cuda:0" instead of gpu?11:34 amGreat question. Short answer: because you can have more than one GPU.
...
pythonmy_list = ["only item"]
my_list[0] # index 0 — the first one
Same idea. cuda:0 means "the CUDA system, GPU number zero."

Clear? If yes, we move to autograd — which is where PyTorch starts to feel genuinely different from NumPy.clear11:35 amAutograd — PyTorch's memory for operations
...
Add this to your tensors.py file and run it:
python# A tensor PyTorch will watch
...
Take a guess, even if you are not sure. Then run it.it should be linear graph, this is all I can say by seeing the equation.and I cant predict that.it says tensor([2.])was I supposed to get the shape of x ?11:42 amYour instinct about it being linear is exactly right — and that is precisely why the answer is 2. Let me show you why.
...
Does the connection between the derivative and the number 2 make sense now? Once you confirm that, we have one small final exercise and then I will update the documents.okay, now I get it. I suppose if I change the equation from y = x _ 3, by calculation the tensor should give me 3. correct ?11:45 amExactly correct. Try it — change the line to y = x _ 3 and run it. Tell me what you get.tensor([3.])11:45 amPerfect. You just verified it yourself — that is the best way to know something is true.
